...
export function loadMixamoAnimation(url, vrm) {
	...
	return loader.loadAsync(url).then((asset) => {
		...
		const vrmHipsY = 0.9;//vrm.humanoid?.getNormalizedBoneNode( 'hips' ).getWorldPosition( _vec3 ).y;
			...
			if (vrmNodeName != null) {
				...
				if (track instanceof THREE.QuaternionKeyframeTrack) {
					...
					const value = track.values.map((v, i) => {
						// Strip X and Z translation for hips to remove root motion but keep Y bounce
						if (vrmBoneName === 'hips' && (i % 3 === 0 || i % 3 === 2)) {
							return 0;
						}
